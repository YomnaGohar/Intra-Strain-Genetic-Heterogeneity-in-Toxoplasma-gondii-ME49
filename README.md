...
Figure 2A was generated by creating a table that counts the number of **SNVs**, **deletions (DEL)**, and **insertions (INS)** in the following VCF files:

- `numt.vcf`
- `tandem.vcf`
- `homopolymer.vcf`

Counts were computed **before and after filtering**, and the final figure was created in Excel.

The resulting table is available in:  
`Data/Before_and_after_graph_filtering.ods`

### 📊 Figure 5C

After running `snakemake --profile profiles/ vcf_analysis`, two key output files will be generated:

- `normalized.vep`
- `normalized.vcf`

These files are used along with the `annotation.gff` file (available in the `resources` folder of the ToxoVar repository) to parse the VEP output.

The following script produces:
- A table (`output.csv`) containing detailed variant information, gene annotations, and orthologous gene IDs from ToxoDB
- A bar plot (`output.pdf`) summarizing variant effects

```bash
python script/parse_vep_file.py normalized.vep normalized.vcf annotation.gff output.csv output.pdf
```
